...
//é um metodo de array, precisa receber como argumento a declaração de uma função. é atraves dessa função que será especificado a forma de execução.  
//Ex.: [1, 2, 3] => [3, 6, 9]

//Porque obter um novo array?
//Para também ter cada item transformado
console.log('============================================ MAP ===========================================')
const numbers = [1, 2, 3]
//console.log(numbers)
...
//Obter no console o array [2,4,6]
//Regra: Essa função sempre deve retornar um valor. Esse valor retornado será adicionado como item do array que o MAP gerar por debaixo dos panos
...
const saleProducts = products.map(product =>{      //Recebendo o proprio objeto.
  if (product.price >= 30) {        //preço >= 30
    //product.price = product.price / 2  //Modificando todo o array, saindo da imultabilidade e ocorrendo multabilidade(mudanças)
    return {
      ...product,                    //SPREAD
...
Quando precisamos obter um novo array com uma quantiade de itens menor que a do array original
...
const numberGreatherThan37 = randomNumbers.filter(item => item > 37) //Irá varrer cada item do array e irá retornar true ou false. Adicionando o item no novo array que será criado. Só riá retornar os valores true a serem inseridos no novo array, senão o item será ignorado. 
...
Quando baseado no array original você precisa "reduzir" o array a um valor. Esse valor pode ser objeto, string, number ou até mesmo um novo arrar, ou seja qualquer valor. Tem versatilidade para gerar um valor qualquer. 
...
const num = [1, 2, 3]
const sumResult = num.reduce((accumlator, item) => accumlator + item, 0) //Segundo argumento é opcional = 0. só usado na primeira execução, Inicial. Sempre colocar 0
//Pode receber também como parametro index e array
//primeira rodada - accumulator = 0 / item = 1
