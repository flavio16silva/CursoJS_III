...
const consertaPalavra = str => {             //Tipo primitivo é imultavel
  return str.replace('serto', 'certo')       //Metodo replace: retorna uma nova string com a expressão que foi passada
  // str[3] = 'c'
  // return str
}
...
/*
 --------------- 2º FORMA
const multiplicar = (a, b, c) => {
if ([a, b, c].includes(undefined)) {       //Includes, esta checando se um item undefined, esta dentro do array
...
const somar = (arg1, arg2, arg3) => {
  ...
  const temNenhumArg = [arg1, arg2, arg3].every(eUndefined)              //every, não invoca função, apenas recebe função. 
...
const saoMesmaVersao = obj => obj.versoes.every(arr => arr[0] === versaoOficial)   //Recebe um objeto como parametro

//   let mesmaVersao = true
//   obj.versoes.forEach(arrVersao => {
//     const versao = arrVersao[0]

//     if (versao !== versaoOficial) {          //Verificado se a versão é diferente da versão oficial
//       mesmaVersao = false
//     }
//   })

//   return mesmaVersao        //returna um boolean
// }
...
//o primeiro 'arquivo', é um parametro
const saoArquivosJS = arquivos.every(arquivo => arquivo.includes('js'))   //Como se lê: Todos os arquivos contem 'js' incluso
//na const acima tanto o include e every retornam um boolean
...
const juntar = (arr1, arr2) => [...arr1, ...arr2] //uso do Spread pelo ...
...
const mudaNomeProduto = produto => { //Parenteses servem para dar prioridade dentro de uma expressão.
